...
const Comp = (props) => {
  ...
  return (
    <div hidden={ getAuthUser?.id !== props.playlist.user.id }>
      <button onClick={ () => handleRemoveImage() } disabled={ !props.playlist.imageUrl || uploadProgress > 0 || loading || data?.updatePlaylist }>
        Remove Image
      </button>

      <form onSubmit={ handleSubmit }>
        <input name={ FORM_FILE } type="file" accept="image/jpeg" required disabled={ uploadProgress > 0 || loading || data?.updatePlaylist }/>
        <button type="submit" disabled={ uploadProgress > 0 || loading || data?.updatePlaylist }>
          Add playlist Image
        </button>
      </form>
